...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    // UniversalRouter is a singleton, but we can keep a reference if we want, or access directly
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // UniversalRouter initialized via shared
    ...
    func setupMenuBar() {
        ...
        if let button = statusItem.button {
            if let icon = IconCatalog.shared.image(named: "icon-inactive.png", resizedTo: NSSize(width: 18, height: 18), template: true) {
                ...
                let fallback = NSImage(systemSymbolName: "network.slash", accessibilityDescription: "FlowRouter")
    ...
    func startServer() {
        // Start Universal Router (8327)
        UniversalRouter.shared.start()
    ...
    private func pollForProxyReadiness(attempts: Int, maxAttempts: Int, intervalMs: Int) {
        if UniversalRouter.shared.isRunning {
            // Success - proceed to start backend
            serverManager.start { [weak self] success in
                DispatchQueue.main.async {
                    if success {
                        self?.updateMenuBarStatus()
                        self?.showNotification(title: "Server Started", body: "FlowRouter is now running")
                    } else {
                        UniversalRouter.shared.stop()
                        self?.showNotification(title: "Server Failed", body: "Could not start backend server on port 8328")
                    }
                }
            }
            return
        ...
        if attempts >= maxAttempts {
            DispatchQueue.main.async { [weak self] in
                UniversalRouter.shared.stop()
                self?.showNotification(title: "Server Failed", body: "Could not start router on port 8327 (timeout)")
            }
            return
        }
    ...
    @objc func copyServerURL() {
        ...
        pasteboard.setString("http://localhost:\(UniversalRouter.shared.proxyPort)", forType: .string)
    ...
    @objc func updateMenuBarStatus() {
        if let serverStatus = menu.item(at: 0) {
            serverStatus.title = serverManager.isRunning ? "Server: Running (port \(UniversalRouter.shared.proxyPort))" : "Server: Stopped"
        }
    ...
    func showNotification(title: String, body: String) {
        ...
        let request = UNNotificationRequest(
            identifier: "io.automaze.flowrouter.\(UUID().uuidString)",
            content: content,
            trigger: nil
        )
